...
Per problem in data/screening_set2.csv:
- 32 cued rollouts, 20 uncued, 8 no-reasoning
- temperature 0.7, top_p 0.95 (stage-1 settings), max_tokens 8192 (nr: 32)
Answers parsed with the canonical format regex. Output: screening2_gen.parquet
...
SAVE_EVERY = 10
OUT = "screening2_gen.parquet"


def main():
    df = pd.read_csv("data/screening_set2.csv")
    done = set()
